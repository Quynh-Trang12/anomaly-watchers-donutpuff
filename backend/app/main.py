...
def _build_risk_factors(
    payload: TransactionInput, probability: float, config: Dict[str, Any]
) -> list[RiskFactor]:
    """
...
def _risk_level(
    probability: float, block_threshold: float, step_up_threshold: float
) -> str:
...
async def lifespan(app: FastAPI):
    ...
    if CONFIG_PATH.exists():
        ...
        app.state.system_configuration = {
            "ml_thresholds": {"block_threshold": 0.8000, "step_up_threshold": 0.4000},
            "business_rules": {
...
async def notify_admin_queue_overflow(
    payload: QueueOverflowNotify, background_tasks: BackgroundTasks
):
    """
...
async def verify_otp(transaction_id: str, user_provided_otp: str):
    ...
    if record.status != TransactionStatusEnum.INITIATED:
        raise HTTPException(
            status_code=400, detail="Transaction is not awaiting verification."
        )

    if record.otp_code != user_provided_otp:
        # Diagnostic log for local dev tracking
        logger.warning(
            "OTP Mismatch for %s: expected %s, got %s",
            transaction_id,
            record.otp_code,
            user_provided_otp,
        )
...
@app.get("/api/transactions/{transaction_id}", response_model=TransactionRecord)
async def get_transaction_by_id(transaction_id: str):
    """
    Retrieves the current status and record for a specific transaction ID.
    Used for polling transaction status during OTP flows.
    """
    record = get_transaction(transaction_id)
    if not record:
        raise HTTPException(status_code=404, detail="Transaction not found.")
...
async def cancel_transaction_otp(transaction_id: str):
    """
    Manually cancels a transaction that is awaiting OTP verification.
    """
    record = get_transaction(transaction_id)
    if not record:
        raise HTTPException(status_code=404, detail="Transaction not found.")
    ...
    if record.status != TransactionStatusEnum.INITIATED:
        raise HTTPException(
            status_code=400, detail="Transaction cannot be cancelled in current state."
        )

    update_transaction_status(transaction_id, TransactionStatusEnum.CANCELLED)

    # Check if this user is a 'consecutive canceller' for security freezing logic (mock logic)
    # In a real app, this would query a counter in Redis/DB
    return {
        "status": "success",
...
async def get_user_status(user_id: str):
    """
    Returns the security status of a user account.
    """
    # Removed frozen account feature
...
async def save_transaction_record(transaction: TransactionRecord):
        ...
        if transaction.status == TransactionStatusEnum.APPROVED:
            try:
                ...
                if transaction.destination_account_id:
                    credit_account_balance(
                        transaction.destination_account_id, transaction.amount
                    )
            except ValueError as ledger_error:
                # Catch technical overdrafts or missing accounts
                logger.warning(
                    "Ledger update failed for new transaction %s: %s",
    ...
    if (
        ...
        and existing_record.status == TransactionStatusEnum.INITIATED
    ):
        ...
        if transaction.otp_code != existing_record.otp_code:
            raise HTTPException(
                status_code=403,
                detail="Unauthorized: Invalid verification code provided for finalization.",
            )
        ...
        if transaction.owner_user_id != existing_record.owner_user_id:
            raise HTTPException(
                status_code=403,
                detail="Unauthorized: Originator mismatch during finalization.",
            )
        ...
        try:
            ...
            logger.warning(
                "Ledger update failed for finalized transaction %s: %s",
                transaction.transaction_id,
                ledger_error,
            )
            raise HTTPException(status_code=400, detail=str(ledger_error))
...
async def debug_fraud_probe():
    ...
    try:
        raw_probability = _predict_probability(
            model_registry["random_forest"], feature_matrix
        )
    except Exception:
    ...
    return {
        ...
        "expected_decision": _risk_level(
            raw_probability, block_threshold, step_up_threshold
        ),
    }
...
async def predict_primary(
    transaction_input: TransactionInput, background_tasks: BackgroundTasks
) -> PredictionOutput:
    """
    ...
    if transaction_input.destination_account_id:
        if transaction_input.destination_account_id == transaction_input.user_id:
            raise HTTPException(
                status_code=400, detail="You cannot transfer money to your own account."
            )

        destination_balance = get_account_balance(
            transaction_input.destination_account_id
        )
    ...
    try:
        ...
        feature_matrix = _align_features(feature_matrix)
        probability_score = _predict_probability(
            model_registry["random_forest"], feature_matrix
        )
    except Exception as execution_exception:
        logger.warning(
            "Inference engine failure, using rule-based fallback: %s",
            execution_exception,
        )
        is_using_fallback = True
        # Rule 6. Fallback Heuristics - Ensures system stays operational if ML engine is offline
        if transaction_input.amount > 150000.0 or (
            transaction_input.amount / (transaction_input.oldbalanceOrg or 1) > 0.9
        ):
    ...
    risk_factors_list = _build_risk_factors(
        transaction_input, probability_score, system_configuration
    )

    transaction_status = TransactionStatusEnum.APPROVED
    operation_explanation = (
        "Everything looks good! Your transaction has been securely processed."
    )
    ...
    if probability_score >= block_threshold:
        ...
        # Security Sequence: Generate OTP -> Fetch recipient email -> Queue Email Task
        recipient_email = get_user_email(transaction_input.user_id) or "user@example.com"
    ...
    if transaction_status == TransactionStatusEnum.APPROVED:
        try:
            ...
            if transaction_input.destination_account_id:
                credit_account_balance(
                    transaction_input.destination_account_id, transaction_input.amount
                )
