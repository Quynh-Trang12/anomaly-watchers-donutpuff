...
export function OTPChallenge({
      ...
      setTimeLeft((prev) => {
        ...
        if (next <= 0) {
          ...
          cancelTransactionOTP(transactionId)
            .catch(() => {});
          setStatus("failed");
          setErrorMessage("Verification time exceeded. Transaction cancelled.");
          if (onCancel) onCancel(); else onFail();
          return 0;
        }
        return next;
      });
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      let respMsg = "Invalid code.";
      if (axios.isAxiosError(error)) {
        respMsg = error.response?.data?.detail || "Invalid code.";
      }
      ...
      if (nextAttempts >= 3) {
        setStatus("failed");
        setErrorMessage("Maximum verification attempts exceeded. Transaction blocked.");
        isFinished.current = true;
        if (timerRef.current) clearInterval(timerRef.current);
        onFail();
      } else {
        setStatus("pending");
        setOtp("");
        setErrorMessage(`${respMsg} (${3 - nextAttempts} attempts remaining)`);
        inputRef.current?.focus();
      }
    }
  };
