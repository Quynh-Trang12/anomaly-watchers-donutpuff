  ...
  const filtered = useMemo(() => {
    return transactions
      .filter((t) => {
        ...
        if (isAdmin && userFilter !== "ALL" && t.owner_user_id !== userFilter)
          return false;
        ...
        {isLoading ? (
          <div className="space-y-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-24 bg-card border animate-pulse rounded-2xl"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-24 text-center bg-card border-2 border-dashed rounded-[2.5rem] mt-4">
            ...
            <p className="text-muted-foreground font-medium">
              Refine your search parameters or process new transactions.
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto">
                    <div
                      className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-colors ${
                        t.type.includes("TRANSFER") || t.type.includes("OUT")
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-mono font-bold tracking-tight">
                        {t.transaction_id}
                      </p>
                      {isAdmin && (
            ...
            >
              ...
              <div className="px-10 pb-10 mt-[-40px]">
                <div className="flex flex-col items-center mb-8">
                  ...
                  <p className="text-muted-foreground font-bold font-mono text-sm mt-2">
                ...
                <div className="grid grid-cols-2 gap-4 mb-10">
                  <div className="p-5 rounded-3xl bg-muted/30 border">
                    ...
                    <p className="text-2xl font-black">
                      {formatCurrencyToUSD(selectedTx.amount)}
                    </p>
                  </div>
                  <div className="p-5 rounded-3xl bg-muted/30 border">
                    <p className="text-[10px] uppercase font-black text-muted-foreground tracking-widest mb-1">
                    ...
                    <p
                      className={`text-2xl font-black ${selectedTx.probability_score > 0.5 ? "text-danger" : "text-primary"}`}
                    >
                ...
                <div className="space-y-4 mb-10 border-t pt-8">
                  ...
                  <DetailLine
                    label="Entry Method"
                    value={selectedTx.type.replace(/_/g, " ")}
                  />
                  <DetailLine
                    label="Originator"
                    value={selectedTx.owner_user_id}
                  />
                  <DetailLine
                ...
                {selectedTx.status === "PENDING_USER_OTP" && (
                  <div className="mb-6 p-4 bg-warning/10 border border-warning/20 rounded-2xl flex items-center gap-3">
                    ...
                    <p className="text-[11px] font-bold text-warning uppercase leading-tight">
                      This transaction is currently awaiting customer
                      verification. Status will update once security protocols
                      are completed.
                    </p>
                  </div>
                )}
...
function DetailLine({
  ...
  return (
    <div className="flex justify-between items-center px-2">
      ...
      <span
        className={`font-black ${highlight ? "text-primary" : "text-foreground"}`}
      >
        {value}
      </span>
    </div>
