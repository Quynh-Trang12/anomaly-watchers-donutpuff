...
export default function Result() {
  ...
  const handleExportReport = () => {
    ...
    const report_lines = [
      ...
      `Status: ${state === "VERIFIED" ? "APPROVED" : state === "CANCELLED" ? "CANCELLED" : state === "REJECTED" ? "REJECTED" : prediction.status}`,
      ...
      ...prediction.risk_factors.map(
        (factor) => `[${factor.severity.toUpperCase()}] ${factor.factor}`,
      ),
      "",
      "--- System Explanation ---",
  ...
  const isApproved =
    (prediction.status === "APPROVED" && state === "INITIAL") ||
    state === "VERIFIED";
  const isBlocked =
    (prediction.status === "BLOCKED" && state === "INITIAL") ||
    state === "REJECTED";
  const isCancelled = state === "CANCELLED";
  const isPending =
    prediction.status === "PENDING_USER_OTP" && state === "INITIAL";
        ...
        <div className="space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground text-lg">
              {state === "VERIFIED"
                ? "Everything looks good! Your identity has been verified and the payment processed."
                : state === "REJECTED"
                  ? "Maximum verification attempts exceeded. For your security, this transaction has been blocked."
                  : state === "CANCELLED"
                    ? "Transaction Cancelled: This request was terminated due to a timeout or user cancellation."
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 mb-6 flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-indigo-600">
                    Action Required: Authenticate Transaction
                  </p>
                  <p className="text-sm text-muted-foreground mt-0.5">
                    A security code has been sent to your simulate email box.
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button
              asChild
              size="lg"
              className="flex-1 h-14 rounded-2xl gap-2 font-bold shadow-lg shadow-primary/20"
            >
              <Link to="/simulate">
